...
"""
function local_clustering_coefficient(g::AS, v::Integer)
...
"""
function local_clustering(g::AS, v::Integer)
    k = degree(g, v)
    k <= 1 && return (0, 0)
...
"""
triangles(g::AS, v::Integer) = local_clustering(g, v)[1]
...
"""
local_clustering_coefficient(g::AS, vlist = vertices(g)) = Float64[local_clustering_coefficient(g, v) for v in vlist]
...
for each `v` in `vlist`.
"""
function local_clustering(g::AS, vlist = vertices(g))
...
"""
triangles(g::AS, vlist = vertices(g)) = local_clustering(g, vlist)[1]
...
"""
function global_clustering_coefficient(g::AS)
    c = 0
    ntriangles = 0
    for v in 1:nv(g)
        neighs = neighbors(g, v)
