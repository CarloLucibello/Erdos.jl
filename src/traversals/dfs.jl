...
"""
function topological_sort_by_dfs(g::ADiGraph)
    T = vertextype(g)
    vcolor = zeros(UInt8, nv(g))
    verts = Vector{T}()
    for v in vertices(g)
...
"""
dfs_tree(g::AGraphOrDiGraph, s::Integer; dir=:out) = tree(dfs_parents(g, s; dir=dir))
...
"""
dfs_parents(g::AGraphOrDiGraph, s::Integer; dir=:out) =
    (dir == :out) ? _dfs_parents(g, s, out_neighbors) :
                    _dfs_parents(g, s, in_neighbors)

function _dfs_parents(g::AGraphOrDiGraph, s::Integer, neighborfn::Function)
    T = vertextype(g)
