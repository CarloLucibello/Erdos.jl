...
"""
    barabasi_albert!(g::AS, n::Integer, k::Integer; seed::Int = -1)
...
"""
function barabasi_albert!(g::AS, n::Integer, k::Integer; seed::Int=-1)
    n0 = nv(g)
    1 <= k <= n0 <= n ||
...
function _create_static_fitness_graph!{T<:Real,S<:Real}(g::AS, m::Int, cum_fitness_out::Vector{T}, cum_fitness_in::Vector{S}, seed::Int)
    rng = getRNG(seed)
...
function blockcounts(sbm::StochasticBlockModel, g::AS)
    return blockcounts(sbm, adjacency_matrix(g))
end

function blockfractions(sbm::StochasticBlockModel, g::Union{AS, AbstractMatrix})
